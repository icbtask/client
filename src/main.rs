        ...
        Some(("todolist", sub_matches)) => {
            ...
            match todolist_subcommand {
                ...
                ("list", _) => {
                    ...
                    let data : Vec<Todolist> = todolists.into_iter().map(|x| Todolist {id : x.todolist_id, name: x.name}).collect();
        ...
        Some(("task", sub_matches)) => {
            ...
            match todolist_subcommand {
                ("add", args) => {
                    ...
                    task::create_task(todolist_id, project, description).await.unwrap();
                    println!("New task created");
                },
                ("list", _) => {
                    let tasks = task::get_tasks().await.unwrap();
                    let data : Vec<Task> = tasks.into_iter().map(|x| Task {id : x.task_id, status: x.status, project: x.project, description: x.description}).collect();
                    println!("{}", Table::new(data));
                }
                ("delete", args) => {
        ...
        Some(("address", sub_matches)) => {
            ...
            match address_subcommand {
                ...
                ("list", _) => {
                    ...
                    let data : Vec<Address> = addresses.into_iter()
                                              .map(|x| Address {
                                                                address: x.address,
                                                                todolist_id: x.todolist_id.unwrap_or_default(),
                                                                allowed_addresses: x.allowed_addresses.into_iter().map(|a| format!("{} @{}\n",a.address, a.username)).collect()})
                                              .collect();
                    println!("{}", Table::new(data));
                }
                ("delete", args) => {
                ...
                ("allow", args) => {
                    ...
                    address::allow_address(address, remote_address).await.unwrap();
                    println!("Address detached successfuly");
                }
                ("revoke", args) => {
                    let address = args.get_one::<String>("address").unwrap();
                    let remote_address = args.get_one::<String>("remote_address").unwrap();
                    address::revoke_address(address, remote_address).await.unwrap();
                    println!("Address detached successfuly");
                }
                (name, _) => {
                    unreachable!("Unknown command {}", name)
