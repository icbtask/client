...
pub async fn create_task(todolist_id: &str, project: &str, description: &str ) -> Result<(), Box<dyn std::error::Error>> {
...
pub async fn get_tasks() -> Result<Vec<Task>, Box<dyn std::error::Error>> {
    ...
    let tasks : Vec<Task>= client.get(url).headers(headers).send().await?.json().await?;
...
pub async fn complete_task(task_id: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = client.patch(url).headers(headers).json(&json).send().await?;
...
pub async fn edit_task(task_id: &str, project: Option<&String>, description: Option<&String>) -> Result<(), Box<dyn std::error::Error>> {
    let client = reqwest::Client::new();
    let url = format!(
        "{}/task/{}",
        envy::from_env::<config::Config>().unwrap().base_url,
